  ...
  export default {
    ...
    methods: {
      ...
      async editPayItem(){
        ...
        const apiUrl = `/edit-pay-item/`+id;
        try {
        let response=await this.$axios.get(apiUrl);
        response=response.data
        this.payItem.name=response.name
        this.payItem.code=response.code

        this.payItem.taxable=response.taxable == true ? ['taxable'] : ''
        this.payItem.pensionable=response.pensionable == true ? ['pensionable'] : '' 
        this.payItem.subject_to_national_insurance=response.code == true ? ['subject_to_national_insurance'] : ''

        this.payItem.payment_type={name:response.payment_type, code:response.payment_type}
      ...
      async updatePayItem(){
        ...
        const apiUrl = `/update-pay-item/`+id;
        try {
        await this.$axios.post(apiUrl,data);
        this.$showToast('success','Pay Item updated successfully.');
        this.isErrors=false
        this.isSuccess=true
        this.goBack()
        } catch (error) {
