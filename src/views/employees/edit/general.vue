  ...
  export default {
    ...
    methods: {
      ...
      async editEmployee(){
        ...
        const apiUrl = `/edit-employee/`+employee_id;
        try {
        let response=await this.$axios.get(apiUrl);
        response=response.data
        this.details=response
        this.details.title={name:response.title, code:response.title}
        this.details.gender={name:response.gender, code:response.gender}
        this.details.ni_category=this.NICategories.find(item => item.code == response.ni_category);
        } catch (error) {
      ...
      async updateEmployee(){
        ...
        const apiUrl = `/update-employee/`+employee_id;
        try {
        let response=await this.$axios.post(apiUrl,data);
        this.$showToast('success','Employee details updated successfully.');
        } catch (error) {
        let errors=error.response.data.errors
        console.log('errors',errors)
        }
